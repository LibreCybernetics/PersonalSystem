...
import dev.librecybernetics.noesis.core.capture.{CaptureProblem, Intent}
import dev.librecybernetics.noesis.core.event.Event
import dev.librecybernetics.noesis.core.kb.*
import dev.librecybernetics.noesis.core.policy.DisclosurePolicy
import dev.librecybernetics.noesis.journal.{InMemoryJournal, Journal, Operation}
...
class KnowledgeBaseSuite extends CatsEffectSuite:
  ...
  test("configured validators reject the scratch closure before the journal is appended"):
    val validator = new StateValidator:
      ...
      def validate(
          _state: dev.librecybernetics.noesis.core.projection.KbState,
          closure: dev.librecybernetics.noesis.reasoner.Closure
      ): List[String] =
        Option
  ...
  test("effective annotations resolve through the cascade, not the raw override"):
    ...
    val book = PolicyBook.empty
      ...
      .withModule(ModuleDefaults("crm", dev.librecybernetics.noesis.logic.Sensitivity.Personal))
    ...
    yield
      ...
      assertEquals(effective.map(_.sensitivity), Some(dev.librecybernetics.noesis.logic.Sensitivity.Personal))
