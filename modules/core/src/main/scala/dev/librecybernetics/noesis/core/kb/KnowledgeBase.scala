package dev.librecybernetics.noesis.core.kb
...
import dev.librecybernetics.noesis.core.capture.{Capture, CaptureProblem, Intent}
import dev.librecybernetics.noesis.core.event.{Event, EventBus, Events}
import dev.librecybernetics.noesis.core.module.ExportContext
import dev.librecybernetics.noesis.journal.{Commit, Journal, Operation}
import dev.librecybernetics.noesis.logic.*
import dev.librecybernetics.noesis.core.policy.*
import dev.librecybernetics.noesis.core.projection.{AxiomRecord, KbState, Projections}
import dev.librecybernetics.noesis.reasoner.query.{BasicGraphPattern, Query, Solution}
import dev.librecybernetics.noesis.reasoner.*
import dev.librecybernetics.noesis.core.verbalize.{Naming, Templates, Verbalizer}
    ...
    NonEmptyList.fromList(operations) match
      ...
      case Some(ops) =>
        val scratch = ops.foldLeft(before): (s, op) =>
          KbState.step(s, dev.librecybernetics.noesis.journal.JournalEntry(s.seq + 1, java.time.Instant.EPOCH, op))
        val scratchClosure =
