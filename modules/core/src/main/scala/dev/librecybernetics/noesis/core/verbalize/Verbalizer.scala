package dev.librecybernetics.noesis.core.verbalize
  ...
  def from(
    ...
    val fromLabels =
      state.activeAxioms.collect:
        case r @ dev.librecybernetics.noesis.core.projection.AxiomRecord(_, Axiom.DataAssertion(s, p, v), _, _, _)
    ...
    val activeObjectLinks = state.activeAxioms.collect:
      case dev.librecybernetics.noesis.core.projection.AxiomRecord(
            _,
