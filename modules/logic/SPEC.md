...
- [EDTF](https://www.loc.gov/standards/datetime/) / ISO 8601-2:2019 ⊘ — a candidate uniform notation for the three located `PartialDate` cases and the separate recurring `MonthDay`. EDTF is maintained by the Library of Congress, freely published, and included in ISO 8601-2 as a profile, so it is the retrievable form of that standard. It is not adopted: three of the four current lexical forms would change, and therefore so would every `AxiomId` that mentions one. The mapping and migration cost are recorded beside D2 in `modules/conformance/DEVIATIONS.md`.
