...
object Literal:
  ...
  /** A literal is always a lexical form with a datatype and an optional language tag.
    *
    * There is one encoding and no discriminator, because the datatype IRI already says what the
    * value is (RDF 1.1 §3.3) — a `type` field beside it would be a second, disagreeable authority
    * on the same question.
    */
  private val decoder: Decoder[Literal] = Decoder.instance: cursor =>
    (cursor.get[String]("lexical"), cursor.get[Iri]("datatype"), cursor.get[Option[String]]("language"))
      .mapN(Literal.apply)
