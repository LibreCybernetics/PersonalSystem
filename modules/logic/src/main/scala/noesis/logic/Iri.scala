...
object Iri:
  /** The IRI denoted by a written name, expanding a bound prefix.
    *
    * `Iri("crm:worksAt")` *is* `Iri("https://noesis.librecybernetics.ws/ns/crm#worksAt")` — compact
    ...
    *
    * It is also the migration. A journal written when compact names were stored decodes through
    * this constructor, so its identifiers expand on the way in; see the journal specification §6.
    *
