...
/** A JSON Lines journal: one atomic commit frame per new line, appended to a file.
  *
  * Legacy one-operation lines remain readable. A new frame carries the entire bundle and a checksum,
  * so a crash cannot turn the accepted prefix of a multi-operation commit into a valid shorter
  * commit. A non-LF-terminated final fragment is discarded on open; corruption anywhere else is
  * fatal (DESIGN journal atomicity and deterministic replay).
  */
        ...
        json.hcursor.get[Int]("formatVersion").toOption match
          case None =>
            json
              .as[JournalEntry]
              .leftMap(error => CorruptJournal(lineNumber, error.getMessage))
              .map(entry => lineNumber -> List(entry))
