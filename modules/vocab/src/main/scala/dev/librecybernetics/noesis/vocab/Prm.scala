...
import dev.librecybernetics.noesis.core.projection.{KbState, Projections}
...
object Prm:
  ...
  def importEvidence(
      contact: Iri,
      intents: NonEmptyList[dev.librecybernetics.noesis.core.capture.Intent]
  ): (Option[String], List[(String, String)]) =
    val list = intents.toList
    val nameRecords = list.collect:
      case dev.librecybernetics.noesis.core.capture.Intent.OpenState(
            `contact`,
            property,
            Node.Ref(name),
            _,
    ...
    val name = list.collectFirst:
      case dev.librecybernetics.noesis.core.capture.Intent.Assert(
    ...
    val methods = list.collect:
      case dev.librecybernetics.noesis.core.capture.Intent.OpenState(method, property, Node.Lit(value), _, _)
